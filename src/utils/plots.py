...
def plot_inference_result(
    ...
    figsize : tuple
        Figure size
    """
    from ..simulator.physics import compute_rmsf, freq_to_lambda_sq, load_frequencies
    ...
    ax3.set_title(
        f"RM Posteriors ({result.model_type}, N={result.n_components})",
        fontsize=12,
        fontweight="bold",
    )
    ax3.legend()
...
def plot_training_summary(
    ...
    legend_elements = [
        Patch(facecolor=colors[i], alpha=0.7, label=mtype)
        for i, mtype in enumerate(model_types)
    ]
    ...
    logger.info("Saved training summary plot")
