...
def _validate_one_model(
    ...
    from ..simulator.prior import (
        get_param_names,
        get_params_per_component,
        sample_prior,
    ...
    if posterior is None:
        print("  ⚠️  ERROR: Posterior is None after loading!")
    ...
    if saved_n_components != n_components:
        print(
            f"  ⚠️  WARNING: Model has N={saved_n_components} but expected N={n_components}"
        )
        return None
    ...
    theta_true_all = sample_prior(
        n_tests, n_components, flat_priors, model_type=model_type
    )
    ...
    for i, name in enumerate(param_names):
        print(
            f"  {name:15s}: MAE = {mae[i]:.4f}, RMSE = {rmse[i]:.4f}, "
...
def _load_posterior(model_path: Path, device: str = "cpu") -> tuple[Any, dict, str]:
    ...
    if posterior is None:
        raise ValueError(
            f"No posterior found in {model_path}. Available keys: {list(data.keys())}"
        )
    ...
    if device != "cpu":
        try:
            ...
            if hasattr(posterior, "_prior"):
                prior_moved = (
                    move_prior_to_device(posterior._prior, device) or prior_moved
                )
            if hasattr(posterior, "prior"):
                prior_moved = (
                    move_prior_to_device(posterior.prior, device) or prior_moved
                )
...
def _plot_recovery(
    ...
    fig.suptitle(
        f"Parameter Recovery: {model_type}, N={n_components}",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
    ...
    with open(summary_path, "w") as f:
        ...
        for result in results:
            ...
            for i, name in enumerate(result["param_names"]):
                f.write(
                    f"  {name:15s}: MAE = {result['mae'][i]:.4f}, "
