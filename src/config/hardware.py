...
class HardwareInfo:
    ...
    def __str__(self):
        ...
        if self.gpu_available:
            ...
            lines.append(
                f"VRAM: {self.gpu_vram_gb:.1f} GB total, {self.gpu_vram_free_gb:.1f} GB free"
            )
...
def detect_hardware(config_hardware=None) -> HardwareInfo:
    ...
    config_workers = (
        getattr(config_hardware, "num_workers", 0) if config_hardware else 0
    )
    ...
    if config_ram > 0:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"RAM detection failed: {e}. Set hardware.ram_gb in config.yaml"
                )
...
def optimize_for_hardware(
    hardware: HardwareInfo | None = None, config=None, verbose: bool = True
) -> OptimizedSettings:
    """
    ...
    if hardware.gpu_available:
        ...
        logger.info(
            f"VRAM: {vram_gb:.1f}GB free → batch_size={settings.training_batch_size}"
        )
    else:
