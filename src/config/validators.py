...
def print_config_summary(config: Configuration):
    ...
    print(
        f"  sigma_phi: [{config.priors.sigma_phi_min}, {config.priors.sigma_phi_max}] rad/m²"
    )
    print(
        f"  delta_phi: [{config.priors.delta_phi_min}, {config.priors.delta_phi_max}] rad/m²"
    )
    ...
    if config.noise.augmentation_enable:
        print(
            f"  Noise range: [{config.noise.augmentation_min_factor}x, {config.noise.augmentation_max_factor}x]"
        )
    ...
    batch_str = (
        ...
        else config.training.training_batch_size
    )
