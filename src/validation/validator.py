...
class Validator:
    ...
    def _load_posterior(self):
        ...
        if self._user_prior_low and self._user_prior_high:
            if len(self._user_prior_low) != self.n_params:
                raise ValueError(
                    f"prior_low has {len(self._user_prior_low)} values, expected {self.n_params}"
                )
            if len(self._user_prior_high) != self.n_params:
                raise ValueError(
                    f"prior_high has {len(self._user_prior_high)} values, expected {self.n_params}"
                )
            self.prior_bounds = {
                "low": self._user_prior_low,
                "high": self._user_prior_high,
            }
            logger.info("  Using user-provided prior bounds")
        elif "prior_bounds" in checkpoint:
            self.prior_bounds = checkpoint["prior_bounds"]
            logger.info("  Prior bounds from checkpoint")
        else:
            raise ValueError(
    ...
    def _simulate_spectrum(self, theta):
        ...
        if self.model_type == "faraday_thin":
            ...
            for i in range(self.n_components):
                ...
                abs_S = np.abs(S)
                depol = np.where(abs_S < 1e-10, np.ones_like(S), (1 - np.exp(-S)) / S)
    ...
    def _save_dat_file(self, case_id, Q_obs, U_obs, weights, sigma):
        ...
        with open(dat_path, "w") as f:
            for i in range(self.n_freq):
                if weights[i] > 0:
                    f.write(
                        f"{freq_hz[i]:.10e} {Q_obs[i]:.10e} {U_obs[i]:.10e} "
                        f"{sigma[i]:.10e} {sigma[i]:.10e}\n"
                    )
        return dat_path

    def _save_case_data(
        self, case_id, theta_true, result, noise_percent, missing_fraction
    ):
    ...
    def _plot_case(
        ...
        ax = axes[0, 1]
        ax.scatter(
            self.lambda_sq[mask], Q_obs[mask], c="blue", s=15, alpha=0.6, label="Q obs"
        )
        ax.scatter(
            self.lambda_sq[mask], U_obs[mask], c="red", s=15, alpha=0.6, label="U obs"
        )
        ...
        ax.set_ylabel("Polarization")
        ax.set_title(
            f"Observed ({noise_percent:.0f}% noise, {missing_fraction * 100:.0f}% flagged)"
        )
        ax.legend()
        ax.grid(True, alpha=0.3)
        ...
        ax.axhline(
            avg_sigma, color="gray", ls=":", lw=1, label=f"±σ (avg={avg_sigma:.4f})"
        )
    ...
    def run_parameter_sweeps(
        self, n_points=20, noise_percent=10.0, missing_fraction=0.1, n_samples=5000
    ):
        ...
        for p in range(self.n_params):
            ...
            for val in tqdm(values, desc=param_name):
                ...
                Q_obs, U_obs, sigma = self._add_noise(
                    Q_true, U_true, P_amp, noise_percent
                )
    ...
    def _plot_sweeps(self, sweep_results, noise_percent, missing_fraction):
        ...
        for idx, (param_name, data) in enumerate(sweep_results.items()):
            ...
            ax.errorbar(
                true, est, yerr=std, fmt="o", capsize=3, alpha=0.7, markersize=5
            )
        ...
        plt.suptitle(
            f"Parameter Sweeps ({noise_percent:.0f}% noise, {missing_fraction * 100:.0f}% flagged)\n"
    ...
    def run_individual_cases(
        self, n_cases=10, noise_percent=10.0, missing_fraction=0.1, n_samples=5000
    ):
        ...
        for i in tqdm(range(n_cases), desc="Cases"):
            ...
            self._save_case_data(
                case_id, theta, result, noise_percent, missing_fraction
            )
            self._plot_case(
                case_id,
                theta,
    ...
    def _plot_summary(self, results, noise_percent, missing_fraction):
        ...
        for p in range(n_params):
            ...
            ax.errorbar(
                true_vals, est_vals, yerr=est_stds, fmt="o", capsize=3, alpha=0.7
            )
        ...
        plt.suptitle(
            f"Individual Cases ({noise_percent:.0f}% noise, {missing_fraction * 100:.0f}% flagged)\n"
            f"Model: {self.model_type}, N={self.n_components}",
            fontsize=12,
            fontweight="bold",
        )
        ...
        plt.axvline(
            np.mean(times), color="red", ls="--", label=f"Mean: {np.mean(times):.1f} ms"
        )
    ...
    def run_validation(
        ...
        self.run_parameter_sweeps(
            n_sweep_points, noise_percent, missing_fraction, n_samples
        )
...
def run_validation(
    ...
    validator.run_validation(
        n_sweep_points, n_cases, noise_percent, missing_fraction, n_samples, seed
    )
    return validator
