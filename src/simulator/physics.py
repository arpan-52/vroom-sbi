...
def compute_rmsf(
    ...
    for i, phi_val in enumerate(phi):
        rmsf[i] = np.sum(
            weights * np.exp(-2j * phi_val * (lambda_sq - lambda_sq_mean))
        ) / K
    
    return rmsf


def get_rmsf_properties(lambda_sq: np.ndarray, weights: np.ndarray = None) -> Dict[str, float]:
    """
...
def compute_faraday_spectrum(
    ...
    for i, phi_val in enumerate(phi):
        F[i] = np.sum(
            weights * P * np.exp(-2j * phi_val * (lambda_sq - lambda_sq_mean))
        ) / K
