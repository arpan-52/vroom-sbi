"""
Weight and noise augmentation for training robust models.
...
def augment_weights_scattered(
    weights: np.ndarray, missing_prob: float = 0.1
) -> np.ndarray:
    """
...
def augment_weights_contiguous_gap(
    weights: np.ndarray, gap_prob: float = 0.3, min_gap: int = 2, max_gap: int = 8
) -> np.ndarray:
    """
...
def augment_weights_large_rfi_block(
    weights: np.ndarray,
...
def augment_weights_noise_variation(
    weights: np.ndarray, variation_scale: float = 0.2
) -> np.ndarray:
    """
    ...
    aug_weights[non_zero_mask] = aug_weights[non_zero_mask] * (
        1 + variation[non_zero_mask]
    )

    # Clip to valid range [0, 1+tolerance]
...
def augment_base_noise_level(
    base_level: float, min_factor: float = 0.5, max_factor: float = 2.0
) -> float:
    """
    Randomly vary the base noise level for training robustness.

    The network learns to handle different overall noise floors.
