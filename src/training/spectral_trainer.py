...
class SpectralShapeTrainer:
    ...
    def _generate_chunks(self, simulator, spectral_cfg, n_simulations: int) -> Path:
        ...
        for ci in range(n_chunks):
            ...
            # Normalise by F(ν₀): simulator always produces F(ν₀)=1 noiseless
            # (log_F0 removed from params).  Just zero out flagged channels so the
            # network sees 0 for missing data, matching inference-time behaviour.
            good_mask = chunk_weights > 0  # (batch, n_freq)
            chunk_x = np.where(good_mask, chunk_x, 0.0)
