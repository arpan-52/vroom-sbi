...
class AsyncChunkStreamer:
    ...
    def __iter__(self):
        ...
        if self.shuffle_chunks:
            chunk_order = random.sample(
                range(len(self.chunk_files)), len(self.chunk_files)
            )
        else:
        ...
        if len(chunk_order) > 1:
            prefetch_future = executor.submit(
                load_chunk, self.chunk_files[chunk_order[1]]
            )
        ...
        try:
            for i, chunk_idx in enumerate(chunk_order):
                ...
                if prefetch_future is not None:
                    current_chunk = prefetch_future.result()  # noqa: F841
...
class StreamingNPETrainer:
    ...
    def _get_sample_from_chunks(
        self, chunk_files: list[Path], n_samples: int = 5000
    ...
    def train(
        ...
        if len(all_chunks) == 1:
            ...
            # Count samples
            train_samples = sum(
                len(torch.load(f, weights_only=True)["theta"]) for f in train_chunks
            )
            val_samples = sum(
                len(torch.load(f, weights_only=True)["theta"]) for f in val_chunks
            )
        ...
        theta_sample, x_sample = self._get_sample_from_chunks(
            train_chunks, n_init_samples
        )
        ...
        logger.info("Starting async streaming training")
...
def train_streaming_npe(
    ...
    **train_kwargs,
    ...
    density_estimator, history = trainer.train(
        chunk_dir=chunk_dir, prior=prior, embedding_net=embedding_net, **train_kwargs
    )
