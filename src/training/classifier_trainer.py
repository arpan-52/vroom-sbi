...
from ..config import ClassifierConfig, Configuration
...
class ClassifierTrainer:
    ...
    def train(
        ...
        if self.best_state is not None:
            ...
            if verbose:
                logger.info(
                    f"Restored best model with {self.best_val_acc:.1f}% accuracy"
                )
...
def train_classifier(
    ...
    if cross_model_training:
        ...
        logger.info(
            f"Cross-model classifier: {n_classes} classes (components {min_components}-{max_components})"
        )
    else:
        n_classes = n_component_range
        logger.info(
            f"Single-model classifier: {n_classes} classes (components {min_components}-{max_components})"
        )
...
def _save_classifier_training_plot(history: dict[str, list[float]], output_dir: Path):
    ...
    logger.info("Saved classifier training plot")
