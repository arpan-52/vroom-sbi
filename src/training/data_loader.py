        ...
        with h5py.File(self.h5_path, "r") as f:
            ...
            for start in range(0, self.n_samples, self.batch_size):
                ...
                theta_t = torch.tensor(
                    theta_batch, dtype=torch.float32, device=self.device
                )
...
def load_simulations(load_path: Path) -> dict[str, Any]:
    ...
    result = {
        ...
        if isinstance(data["spectra"], torch.Tensor)
        else data["spectra"],
        "weights": data["weights"].numpy()
        if isinstance(data["weights"], torch.Tensor)
        else data["weights"],
        "theta": data["theta"].numpy()
        if isinstance(data["theta"], torch.Tensor)
        else data["theta"],
        ...
        for model_type in model_types:
            for n_comp in range(min_components, max_components + 1):
                ...
                if not sim_path.exists():
                    sim_path = (
                        simulations_dir / f"simulations_{model_type}_n{n_comp}.pkl"
                    )
                ...
                logger.info(
                    f"Loaded {n_samples} samples for {model_type} n={n_comp} (class {class_idx})"
                )
                class_idx += 1
    else:
