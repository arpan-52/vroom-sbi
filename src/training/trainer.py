...
try:
    ...
    try:
        from sbi.neural_nets import posterior_nn  # noqa: F401
    except ImportError:
        pass
...
class SBITrainer:
    ...
    def train_model(
        ...
        logger.info(
            f"  Hidden: {arch_config.hidden_features}, Transforms: {arch_config.num_transforms}"
        )
        ...
        if training_history.get("val_loss"):
            ...
            if val_losses:
                ...
                logger.info(
                    f"Best validation loss: {best_val:.4f} (epoch {best_epoch})"
                )
        ...
        result = TrainingResult(
            ...
            final_val_loss=training_history.get("val_loss", [None])[-1]
            if training_history.get("val_loss")
            else None,
        ...
        if "train_loss" in training_history:
            for i, train_loss in enumerate(training_history["train_loss"]):
                val_loss = training_history.get(
                    "val_loss", [None] * len(training_history["train_loss"])
                )[i]
                result.add_metric(
                    TrainingMetrics(
                        epoch=i,
        ...
        # Save training plots
        if training_history and (
            training_history.get("train_loss") or training_history.get("val_loss")
        ):
    ...
    def _generate_simulation_chunks(
        ...
        for chunk_idx in range(n_chunks):
            ...
            logger.info(
                f"Generating chunk {chunk_idx + 1}/{n_chunks} ({this_chunk_size:,} samples)..."
            )
                ...
                if key in summary and summary[key] is not None:
                    ...
                    if isinstance(values, list | tuple) and len(values) > 0:
                        if "log_prob" in key:
                            ...
                            history["train_loss"] = [to_float(x) for x in values]
                        logger.info(
                            f"Found training data in '{key}': {len(history['train_loss'])} epochs"
                        )
                        break
                ...
                if key in summary and summary[key] is not None:
                    ...
                    if isinstance(values, list | tuple) and len(values) > 0:
                        if "log_prob" in key:
                            ...
                            history["val_loss"] = [to_float(x) for x in values]
                        logger.info(
                            f"Found validation data in '{key}': {len(history['val_loss'])} epochs"
                        )
                        break
    ...
    def _save_posterior(
        ...
        try:
            ...
            if hasattr(prior, "base_dist"):
                ...
                low, high = self.config.priors.get_bounds_for_model(
                ...
                logger.warning(
                    "Could not extract prior bounds from SBI object, using config"
                )
        except Exception as e:
...
def train_model(
    config: Configuration, model_type: str, n_components: int, **kwargs
) -> TrainingResult:
    """
