...
class ModelCheckpoint:
    ...
    # SBI-specific
    prior_bounds: dict[str, list[float]] | None = None
    embedding_net_state: dict[str, torch.Tensor] | None = None
...
class CheckpointManager:
    ...
    def _get_checkpoint_path(
        self, model_type: str, n_components: int, epoch: int, is_best: bool = False
    ) -> Path:
        ...
        return (
            self.checkpoint_dir
            / f"checkpoint_{model_type}_n{n_components}_epoch{epoch:04d}.pt"
        )
    ...
    def load_checkpoint(
        ...
        if load_best:
            checkpoint_path = self._get_checkpoint_path(
                model_type, n_components, 0, is_best=True
            )
        ...
        checkpoint = ModelCheckpoint(
            ...
            timestamp=datetime.fromisoformat(
                save_dict.get("timestamp", datetime.now().isoformat())
            ),
    ...
    def list_checkpoints(self, model_type: str | None = None) -> list[dict[str, Any]]:
        ...
        pattern = (
            "checkpoint_*.pt" if model_type is None else f"checkpoint_{model_type}_*.pt"
        )
        ...
        return sorted(
            checkpoints, key=lambda x: (x["model_type"], x["n_components"], x["epoch"])
        )
...
def save_training_plots(
    ...
    if has_train:
        ...
        ax1.plot(
            epochs, train_loss, "b-", linewidth=2, label="Training Loss", alpha=0.8
        )
    ...
    if has_val:
        ...
        ax1.plot(
            epochs_val, val_loss, "r-", linewidth=2, label="Validation Loss", alpha=0.8
        )
        ...
        ax1.scatter(
            [best_idx + 1], [val_loss[best_idx]], color="g", s=100, zorder=5, marker="*"
        )
    ...
    if has_val and len(val_loss) > 1:
        ...
        if window > 1:
            ...
            ax2.plot(
                ...
                "r-",
                linewidth=2,
                label=f"Smoothed Val (window={window})",
            )

        # Calculate improvement rate
        improvements = np.diff(val_loss)
        ax2.bar(
            range(2, len(val_loss) + 1),
            -improvements,
            alpha=0.5,
            color="blue",
            label="Epoch Improvement",
        )
