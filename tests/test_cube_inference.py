...
from tests.conftest import (
    N_FREQ, N_DEC, N_RA, GT_RM, GT_AMP, GT_CHI0,
...
def test_cube_inference_iquv(q_cube, u_cube):
    ...
    with patch.object(engine, 'infer', return_value=(fake_result, {'faraday_thin_n1': fake_result})):
...
def test_cube_inference_user_mask(q_cube, u_cube):
    ...
    assert np.all(np.isfinite(rm_map[:, N_RA // 2:])), "unmasked pixels should be finite"
...
def test_cube_inference_noise_weights(q_cube, u_cube):
    ...
    zero_weight_calls = [w for w in captured_weights if w is not None and np.all(w == 0)]
